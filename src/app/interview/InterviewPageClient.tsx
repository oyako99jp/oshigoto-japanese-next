...
async function loadProgress() {
  try { const r = await window.localStorage.get("interview_progress"); return r ? JSON.parse(r.value) : DEFAULT_PROGRESS; }
  catch { return DEFAULT_PROGRESS; }
}
async function saveProgress(p: { completed: number[]; inProgress: number[] }) {
  try { await window.localStorage.set("interview_progress", JSON.stringify(p)); } catch {}
}
...
function SectionTab({ sections, active, onChange, isPC }) {
  return (
    <div style={{ borderBottom: "2px solid #dddddd", display: "flex", gap: 0, marginBottom: 24, overflowX: "auto", overflowY: "hidden", WebkitOverflowScrolling: "touch", whiteSpace: "nowrap" }}>
      {sections.map(s => (
        <button key={s.id} onClick={() => onChange(s.id)} style={{ background: "none", border: "none", borderBottom: active === s.id ? "2px solid #e61923" : "2px solid transparent", marginBottom: -2, padding: "10px 20px", fontWeight: active === s.id ? 700 : 400, color: active === s.id ? "#e61923" : "#555555", cursor: "pointer", fontSize: isPC ? 13 : 12, display: "inline-block", whiteSpace: "nowrap", flexShrink: 0 }}>{s.label}</button>
      ))}
    </div>
  );
}
...
function Header({ isPC }) {
  ...
  return (
    <div style={{ width: "100%", background: "#e61923", color: colors.white }}>
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: isPC ? "18px 24px" : "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <h1 style={{ margin: 0, fontSize: isPC ? 22 : 18, fontWeight: 800 }}>就職・転職面接のための日本語</h1>
    ...
    <div style={{ background: colors.white, borderRadius: 0, padding: isPC ? 28 : 20, boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: 13, color: colors.gray }}>Job Interview</span>
...
function HeroSection({ onStart }: { onStart: () => void }) {
  return (
    <div style={{ width: "100%", backgroundImage: "url('/hero_interview.png')", backgroundSize: "cover", backgroundPosition: "center", padding: "80px 24px", textAlign: "center", position: "relative" }}>
      <div style={{ position: "absolute", background: "rgba(180, 10, 20, 0.65)", inset: 0 }} />
...
function AboutCourse() {
  return (
    <div style={{ marginBottom: 24, padding: "18px 22px", border: "1px solid #dddddd", background: "#fef8f8" }}>
      <h3 style={{ margin: "0 0 12px", fontSize: 16, fontWeight: 700, color: "#e61923" }}>このコースについて / About This Course</h3>
      <p style={{ margin: 0, fontSize: 14, color: "#555555", lineHeight: 1.8 }}>
        おしごとJapaneseの面接コースは、日本での就職・転職活動のための面接日本語を学べるコースです。自己PR・志望動機・よくある質問・ビジネス敬語・逆質問など、面接の全ステップをカバーします。JLPT N2以上を目標とする方に最適です。
      </p>
    </div>
  );
}
...
function TopPage({ onSelectUnit, progress }) {
  ...
  const width = useWindowWidth();
  const isPC = width >= 768;
  const pct = Math.round((progress.completed.length / 10) * 100);
  ...
  const UnitCard = ({ unit }) => {
    ...
    const border = done ? "#86efac" : inProg ? "#fde047" : unit.available ? "#fecaca" : colors.grayLight;
    ...
    return (
      <div onClick={() => unit.available && onSelectUnit(unit)} style={{ background: bg, border: `1px solid ${border}`, borderRadius: 0, padding: "13px 15px", cursor: unit.available ? "pointer" : "default", display: "flex", alignItems: "center", gap: 12, transition: "background 0.15s" }}
        onMouseEnter={e => { if (unit.available) { e.currentTarget.style.background = done ? "#bbf7d0" : inProg ? "#fef08a" : "#fce4e6"; }}}
    ...
    <div style={{ background: colors.white, borderRadius: 0, padding: isPC ? 28 : 20, boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 13, color: colors.gray }}>{progress.completed.length} / 10 units</span>
        </div>
        <div style={{ background: colors.grayLight, borderRadius: 0, height: 10, overflow: "hidden" }}>
          <div style={{ width: `${pct}%`, height: "100%", background: "linear-gradient(90deg,#e61923,#c0141e)", borderRadius: 0, transition: "width 0.6s" }} />
        </div>
        <div style={{ textAlign: "right", marginTop: 4, fontSize: 12, color: colors.textLight }}>{pct}% complete</div>
      </div>
      <h2 style={{ fontSize: 15, fontWeight: 700, color: colors.text, marginBottom: 14, marginTop: 0 }}>Units — Job Interview</h2>
  ...
  const sidebar = (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ background: colors.white, borderRadius: 0, padding: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.07)" }}>
        ...
        {[{ label: "Completed", value: progress.completed.length, color: "#22c55e" }, { label: "Remaining", value: 10 - progress.completed.length, color: colors.primaryLight }, { label: "Total Units", value: 10, color: colors.gray }].map(s => (
        ...
        <div style={{ borderTop: `1px solid ${colors.grayLight}`, paddingTop: 12, fontSize: 12, color: colors.textLight, textAlign: "center" }}>Target: JLPT N2+ · Job Interviews</div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", width: "100vw", background: "#f5f5f5", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <Header isPC={isPC} />
      <HeroSection onStart={() => router.push("/interview")} />
